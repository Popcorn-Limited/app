...
import useNamedAccounts from "lib/utils/hooks/useNamedAccounts";
import { useMultiStatus } from "lib/utils/hooks/useMultiStatus";
...
export const useTvl: Pop.Hook<BigNumberWithFormatted> = ({ chainId, address, resolver, enabled }: Props) => {
  ...
  const balance = !!_tvlResolver ? secondaryBalance : primaryBalance?.value;
