...
export default function VeRewards(): JSX.Element {
  ...
  useEffect(() => {
    if (apy === 0) {
      getVeApy({ chain: goerli, address: FEE_DISTRIBUTOR, token: "0x2D9B33e9918Dce388d1Cb8Bf09D4E827b899e9d9" }).then(res => setApy(res))
    }
  }, [apy])
  ...
  useEffect(() => {
    async function getClaimableReward() {
      ...
      getClaimableVeReward({ chain: goerli, address: FEE_DISTRIBUTOR, user: account as Address, token: WETH }).then(res => setClaimableVeReward(res))
    }
  ...
  return (
    <div className="lg:w-1/2 bg-[#FAF9F4] border border-[#F0EEE0] rounded-3xl p-8 text-primary">
      <h3 className="text-2xl pb-6 border-b border-[#F0EEE0]">Total veVCX Rewards</h3>
      ...
      <span className="flex flex-row items-center justify-between">
        ...
        <p className="font-bold">{parseFloat(formatEther(claimableVeReward)).toFixed(3)} wETH</p>
      </span>
      <div className="mt-5 flex flex-row items-center justify-between space-x-8">
        <MainActionButton label="Claim WETH" handleClick={() => claimTokens()} disabled={Number(claimableVeReward) === 0} />
      </div>
    </div>
  )
}