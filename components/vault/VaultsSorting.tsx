    ...
    return (
        <div className={`relative ${className}`}>
            <button
                ...
                className="w-full h-[58px] py-[14px] px-6 gap-2 flex flex-row items-center justify-between rounded-lg border border-customLightGray"
            >
                <div className="flex items-center">
                    <p className="text-primary">Sorting</p>
                </div>
                <SwitchIcon className="w-5 h-5 text-primary" color={"#645F4B"} size={"21px"} />
            </button>
            {openFilter && !isLessThenMdScreenSize && (
                <div ref={dropdownRef} className="hidden md:block absolute w-[180px] p-[10px] border border-customLightGray top-16 bg-[#FAF9F4] rounded-lg right-0">
                    <button
                        className={`
                            py-2 w-full 
                            cursor-pointer
                            ...
                            ease-in-out 
                            duration-250 
                            ${currentSortingType === VAULT_SORTING_TYPE.mostTvl ? 'border-2 border-warmGray' : 'bg-[#FAF9F4]'}
                        `}
                    ...
                    <button
                        className={`
                            ...
                            ease-in-out 
                            duration-250
                            ${currentSortingType === VAULT_SORTING_TYPE.lessTvl ? 'border-2 border-warmGray' : 'bg-[#FAF9F4]'}
                        `}
                    ...
                    <button
                        className={`
                            ...
                            ease-in-out 
                            duration-250
                            ${currentSortingType === VAULT_SORTING_TYPE.mostvAPR ? 'border-2 border-warmGray' : 'bg-[#FAF9F4]'}
                        `}
                    ...
                    <button
                        className={`
                            ...
                            ease-in-out 
                            duration-250
                            ${currentSortingType === VAULT_SORTING_TYPE.lessvAPR ? 'border-2 border-warmGray' : 'bg-[#FAF9F4]'}
                        `}
