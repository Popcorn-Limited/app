    ...
    <>
      ...
      <div className="flex justify-between items-center mt-2 w-full">
        <div className="flex items-center ml-1 gap-2 cursor-pointer group/max" onClick={onMaxClick}>
          {allowInput && <>
            <div className=" text-secondaryLight group-hover/max:text-primary mb-2">
            ...
            <p className="text-secondaryLight group-hover/max:text-primary mb-2.5">
              {`${selectedToken?.balance ? formatNumber(Number(selectedToken?.balance) / (10 ** (selectedToken?.decimals as number))) : "0"}`}
            </p>
          </>}
        </div>
        {getTokenUrl && (
